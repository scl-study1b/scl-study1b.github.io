...
slides.intentionR = slide({
  ...
  /* trial information for this block
   (the variable 'stim' will change between each of these values,
    ...
    // randomize order
  present : randomSort([
    {sentence: "How likely is it that Brock-bug intentionally opened the door?"}, //intention1
    {sentence: "How likely is it that Brock-bug knew how to open the door?"}, //knowledge
    {sentence: "Was Brock-bug aiming to open the door?"}, //goal
    {sentence: "Was Brock-bug aiming to clean the room?"}, //goalClean
    {sentence: "Are Brock-bug's actions intentional?"} //intention2
  ]),
  ...
  present_handle : function(stim) {
    ...
    document.getElementById('opt8IR').checked = false;
    document.getElementById('opt9IR').checked = false;
...
slides.attention2R =  slide({ //check this!!!
  name : "attention2R",
...
function sendDataToServer(data) {
	$.ajax({
    url: 'https://script.google.com/macros/s/AKfycbwmp-O4ycnCxQ326ytgO_3_8vFxZxks9LH6AJwClL7bsWfn1wjDiPv_ugeVaAdFBSGLFw/exec',
		type: 'post',
    data: data,
    complete: function(){
      window.location.replace("https://app.prolific.co/submissions/complete?cc=4E9A47B4") // redirect to prolific
    } 
	});
}
